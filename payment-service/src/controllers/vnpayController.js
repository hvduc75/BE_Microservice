  ...
  if (secureHash === signed) {
    ...
    try {
      if (responseCode === "00") {
        ...
        await paymentService.updatePayment(orderId, "failed");
        return res.redirect(
          "http://localhost:3000/checkout/status?status=failed"
        );
      }
    } catch (error) {
      console.error(error);
...
const refund = async (req, res) => {
  try {
    ...
    let refundCheckResult = await paymentService.checkRefundOrder(orderId);
